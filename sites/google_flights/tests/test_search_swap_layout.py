...
Run with:
    ...
    python3 -m unittest discover -s sites/google_flights/tests -v
"""
...
class SearchSwapLayoutTests(unittest.TestCase):
    def test_origin_destination_are_grouped_apart_from_dates(self):
        parser = _SearchFormParser()
        parser.feed(INDEX.read_text())
        self.assertEqual(
            ["search-route-fields", "search-dates"], parser.row_children
        )
        self.assertEqual(["from", "search-swap", "to"], parser.route_children)
        self.assertFalse(parser.dates_inside_route)

    def test_swap_is_positioned_on_the_route_group_seam(self):
        css = CSS.read_text()
        row = _rule_body(css, ".search-row")
        route = _rule_body(css, ".search-route-fields")
        swap = _rule_body(css, ".search-swap")

        self.assertNotIn("position: relative", row)
        self.assertIn("position: relative", route)
        self.assertIn("grid-template-columns", route)
        self.assertIn("position: absolute", swap)
        self.assertIn("left: 50%", swap)
        self.assertIn("top: 50%", swap)

    def test_mobile_hides_swap_and_stacks_route_fields(self):
        mobile = _mobile_block(CSS.read_text())
        self.assertIn(".search-swap { display: none; }", mobile)
        self.assertIn("flex-direction: column", mobile)
        self.assertIn(".search-route-fields", mobile)
